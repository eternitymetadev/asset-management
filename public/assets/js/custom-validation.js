...
jQuery(document).ready(function () {
    ...
    //////////////////
    //// sigle lead assign user

    jQuery(document).on('click', '.singleassetassign', function(){
        var getid = $(this).attr('data-assetid');
        var name = $(this).attr('data-name');
        // var leadtype = $(this).attr('data-leadtype');
        $('.singleassignuser').attr('data-id',getid);
        $('.singleassignuser').attr('data-name',name);
        // $('.singleassignuser').attr('data-leadtype',leadtype);
      });
  
  
  
  
      jQuery(document).on('click', '.singleassignuser', function(){
        // var singleleaduserrole = jQuery(".singleuser option:selected").attr('role');
        // var getleadtype    =  jQuery(this).attr('data-leadtype');
        // if(getleadtype == 2 && singleleaduserrole == 3){
        //   var segmentName = $('#stop-assign-tradelead-broker').attr('type');
        //   $("#stop-assign-tradelead-broker").html('Trade '+segmentName+' can not be assigned to broker');
        //   return false;
        // }
        // $("#stop-assign-tradelead-broker").html('');
        var singleleaduser = jQuery(".singleuser option:selected").val();
        if(singleleaduser == ''){
          $("#not-assign-empty").html('Please select user to assign.');
          return false;
        }
        $("#not-assign-empty").html('');
        jQuery('#showassignasset').modal('show');
        var url  =  jQuery(this).attr('data-action');
        var assetid    =  jQuery(this).attr('data-id');
        // var showassignarray = [];
          //var search =  jQuery('#search').val();
          //var singleleaduser = jQuery('.assignuser').val();
  
          var selectusername =  jQuery(".singleuser option:selected").text();
          //jQuery('.singleassignuser').text(selectusername);
          jQuery("#showassigned").html('');
  
          var name = jQuery(this).attr('data-name');
          var nameText =  '<tr><td>'+ name +'</td>';
          nameText +='<td>'+selectusername+'</td></tr>';
          jQuery("#showassigned").append(nameText);
          var singleuser = 'singleuser';
  
          var getpagetext =  jQuery('.page-item.active span').text();
          jQuery(document).off('click','.assignasset').on('click', '.assignasset', function(){
            jQuery.ajax({
              type      : 'get',
              url       : url,
              data      : {singleuser:singleuser,singleleaduser:singleleaduser,assetid:assetid,page:getpagetext},
              headers   : {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
              },
              dataType  : 'json',
              success:function(response){
                ...
                //   jQuery('#commonconfirm').modal('hide');
                  jQuery("#showassignasset").modal("hide");
                  jQuery("#singleassign_user").modal("hide");
                }
              }
            });
          });
